    ...
    <div className="auth-overlay" onClick={handleOverlayClick} role="dialog" aria-modal="true">
      <div className="auth-modal">
        <div className="auth-modal__header">
          ...
          <button className="auth-modal__close" type="button" onClick={onClose} aria-label={t('auth.close')}>
            <FaTimes />
          </button>
        </div>

        <div className="auth-modal__title-wrap">
          <h2 className="auth-modal__title">{isLogin ? t('auth.login') : t('auth.register')}</h2>
          <p className="auth-modal__subtitle">{isLogin ? t('auth.loginWelcome') : t('auth.registerWelcome')}</p>
        </div>
        ...
        <button className="auth-modal__btn-google" type="button" onClick={handleGoogleLogin}>
        ...
        <p className="auth-modal__switch">
          {isLogin ? (
            <>
              {t('auth.noAccount')}{' '}
              <button type="button" className="auth-modal__switch-link" onClick={() => setMode('register')}>
                {t('auth.register')}
              </button>
            </>
          ) : (
            <>
              {t('auth.haveAccount')}{' '}
              <button type="button" className="auth-modal__switch-link" onClick={() => setMode('login')}>
                {t('auth.login')}
              </button>
            </>
          )}
